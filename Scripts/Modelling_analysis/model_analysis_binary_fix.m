...
%% get fix and adapt
for i = 1:length(sub_id)
    model_sub_fix{i,1} = model_sub{i}(model_sub{i}(:,5)<3,:);
    model_sub_adapt{i,1} = model_sub{i}(model_sub{i}(:,5)==4 | model_sub{i}(:,5)==3,:);
end
test = cellfun('isempty',model_sub_fix);
for i = 1:length(test)
    if test(i) == 0
       model_sub_fix{i,1}(2:end,6) = 0;
    end
end
clear test

test = cellfun('isempty',model_sub_adapt);
for i = 1:length(test)
    if test(i) == 0
       model_sub_adapt{i,1}(2:end,6) = 0;
    end
end
clear test
%%
data_con = model_sub_fix(~cellfun('isempty',model_sub_fix));
for i = 1:size(data_con,1)
    index = isnan(data_con{i}(:,3));
    data_con{i,1}(index,3) = 0;
%     data_con{i,1}(2:end,6) = 0;
    clear index;
    fix_id(i,1) = data_con{i}(1,1);
end
%%
data_con_adap = model_sub_adapt(~cellfun('isempty',model_sub_adapt));
for i = 1:size(data_con_adap,1)
    index = isnan(data_con_adap{i}(:,3));
    data_con_adap{i,1}(index,3) = 0;
%     data_con{i,1}(2:end,6) = 0;
    clear index;
    adap_id(i,1) = data_con_adap{i}(1,1);
end
%%
% co_sub(:,1) = cellfun('isempty',model_sub_fix);
% co_sub(:,2) = cellfun('isempty',model_sub_adapt);
% co_id = sum(co_sub,2) == 0;
% 
% for i = 1:length(co_id)
%     if co_id(i) == 1
%     data_con{i,1} = model_sub_fix{i};%model_sub_fix/model_sub_adapt
%     data_con{i}(1,8) = .5;%0.5 or 2.5
%     end
% end
%% everyone
% for i = 1:size(data_con_adap,1)
%     [LIA,LOCB] = ismember(adap_id(i,1),fix_id);
%     if LIA == 1
%        data_con{LOCB} = [data_con{LOCB};data_con_adap{i}];
%     else
%         data_con{size(data_con,1)+1,1} = data_con_adap{i}; 
%     end
% end
%% have both
% for i = 1:size(data_con_adap,1)
%     [LIA,LOCB] = ismember(adap_id(i,1),fix_id);
%     if LIA == 1
%        data_con2{LOCB,1} = [data_con{LOCB};data_con_adap{i}];
%     end
% end
% clear data_con
...
n1 = 8; % for plots
n2 = 8;
which_kids = [1:size(data_con,1)];

for i_mod = [1 2 3 4 5 6]
...
%  for i_sub=1
   % for i_sub = which_kids
        ...
        A.investor = data_con{ which_kids(i_sub), 1 }( :, 2); % my decision
        A.trustee = data_con{ which_kids(i_sub), 1 }( :, 3); % trustee 3outcome/9trustee share
        A.reset_sub = data_con{ which_kids(i_sub), 1 }( :, 7); % Reset social/non etc 6;cc/sc etc 7
        A.condition = data_con{ which_kids(i_sub), 1 }( :, 5); % condition
%         A.prior = data_con{ which_kids(i_sub), 1 }( 1, 8 ); % prior
        A.prior = 0.5; % reset
...
%             to_plot = fitmodel_mle_rl_BIN_bound_to_plot(A.trustee, A.reset_sub, A.investor, A.condition, A.prior, estimates_m{ i_mod }(i_sub,:));
            ...
            n = size(A.investor,1);
            % note: reset is always at 0.5, which is the natural midpoint
            [estimates_m{ i_mod }(i_sub,:), model, fval, exitflag(i_sub,i_mod), A.v_out, A.delta_out] = fitmodel_mle_rl_BIN_bound_onepar_fix( A.trustee, A.reset_sub, A.investor, A.condition, A.prior  ); % xdata1 is outcome
            % Note: minimization not for SSE (sum of squared errors) but
            % for "LLE" negative log-likelhood (basically difference bt 
            % "normal" regression & logistic regression
            % This changes formulas for AIC/BIC
...
%             n = size(A.investor,1);
%             % note: reset is always at 0.5, which is the natural midpoint
%             [estimates_m{ i_mod }(i_sub,:), model, fval, exitflag(i_sub,i_mod), A.v_out, A.delta_out] = fitmodel_mle_rl_BIN_bound_onepar_noprior_nodecay( A.trustee, A.reset_sub, A.investor, A.condition, A.prior  ); % xdata1 is outcome
%             % Note: minimization not for SSE (sum of squared errors) but
%             % for "LLE" negative log-likelhood (basically difference bt
%             % "normal" regression & logistic regression
%             % This changes formulas for AIC/BIC
...
%             to_plot = fitmodel_mle_tit_f_tat_fixed_to_plot(A.trustee, A.reset_sub, A.investor, estimates_m{ i_mod }(i_sub,:));
          ...
          elseif i_mod == 5
            n = size(A.investor,1);
            [estimates_m{ i_mod }(i_sub,:), model, fval, exitflag(i_sub,i_mod), A.v_out] =  fitmodel_mle_tit_f_tat_onepar_fix_keep( A.trustee, A.reset_sub, A.investor, A.condition, A.prior); % xdata1 is outcome
            [LLE(i_sub,i_mod), FittedCurve2] = model(estimates_m{ i_mod }(i_sub,:)); 
            AIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*1; % Akaike Information Criterion  % k1 = 1; % number of free parameters
            BIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*log(n)/2; % Bayesian Information Criterion
%             to_plot = fitmodel_mle_tit_f_tat_fixed_to_plot(A.trustee, A.reset_sub, A.investor, estimates_m{ i_mod }(i_sub,:));
          to_plot = A.v_out;
          ...
          elseif i_mod == 6
            n = size(A.investor,1);
            [estimates_m{ i_mod }(i_sub,:), model, fval, exitflag(i_sub,i_mod), A.v_out] =  fitmodel_mle_tit_f_tat_onepar_nodecay( A.trustee, A.reset_sub, A.investor, A.condition, A.prior ); % xdata1 is outcome
            [LLE(i_sub,i_mod), FittedCurve2] = model(estimates_m{ i_mod }(i_sub,:)); 
            AIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 2*1; % Akaike Information Criterion  % k1 = 1; % number of free parameters
            BIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 2*log(n)/2; % Bayesian Information Criterion
%             to_plot = fitmodel_mle_tit_f_tat_fixed_to_plot(A.trustee, A.reset_sub, A.investor, estimates_m{ i_mod }(i_sub,:));
...
%           to_plot = A.v_out;
          ...
          elseif i_mod == 7
            n = size(A.investor,1);
            [estimates_m{ i_mod }(i_sub,:), model, fval, exitflag(i_sub,i_mod), A.v_out] =  fitmodel_mle_onepar_fix_keep( A.trustee, A.reset_sub, A.investor, A.condition, A.prior); % xdata1 is outcome
            [LLE(i_sub,i_mod), FittedCurve2] = model(estimates_m{ i_mod }(i_sub,:)); 
            AIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*1; % Akaike Information Criterion  % k1 = 1; % number of free parameters
            BIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*log(n)/2; % Bayesian Information Criterion
%             to_plot = fitmodel_mle_tit_f_tat_fixed_to_plot(A.trustee, A.reset_sub, A.investor, estimates_m{ i_mod }(i_sub,:));
...
%  ylim([1000 1800]);
 xlabel('Delta BIC')
 yticklabels({sprintf('RL full model'),sprintf('RL fix'),sprintf('RL no decay'),...
     sprintf('t4t full model'),sprintf('t4t fix'),sprintf('t4t no decay')})
 title('Non-adaptive')
% xtickangle(45)
%%
clear BMS
