...
%           elseif i_mod == 7
%             startpoint = rand(numinit,1).*40 -20;
%             for run_idex = 1:numinit
%                  n = size(A.investor,1);
%                 [estimates_m{ run_idex,i_mod }(i_sub,:), model, fval, exitflag{run_idex}(i_sub,i_mod), A.v_out{run_idex}] =  fitmodel_mle_onepar_fix_keep( A.trustee, A.reset_sub, A.investor, A.condition, A.prior,startpoint(run_idex)); % xdata1 is outcome
%                 [LLE_all(run_idex,i_sub,i_mod), FittedCurve2] = model(estimates_m{ run_idex,i_mod }(i_sub,:)); 
%             end
%             [LLE(i_sub,i_mod),min_idex]=min(LLE_all(:,i_sub,i_mod));
%             AIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*1; % Akaike Information Criterion  % k1 = 1; % number of free parameters
%             BIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*log(n)/2; % Bayesian Information Criterion
% %             to_plot = fitmodel_mle_tit_f_tat_fixed_to_plot(A.trustee, A.reset_sub, A.investor, estimates_m{ i_mod }(i_sub,:));
%           to_plot = A.v_out{min_idex};
