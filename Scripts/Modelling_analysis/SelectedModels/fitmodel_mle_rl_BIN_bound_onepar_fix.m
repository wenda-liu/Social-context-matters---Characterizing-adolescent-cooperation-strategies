function [estimates, model, fval, exitflag, v_vec, delta] = fitmodel_mle_rl_BIN_bound_onepar_fix(trustee, reset_sub, ydata, condition, prior) % trustee is outcome, xdat2 is for resetting
% Call fminsearch with a random starting point.
% start_point = rand(1);
start_point = [0.5, 0.1]; % i.e., HERE p(1) = alpha % p(2) = beta (inverse temperature) 
model = @expfun;
