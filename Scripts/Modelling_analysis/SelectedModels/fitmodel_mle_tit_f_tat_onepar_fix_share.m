function [estimates, model, fval, exitflag, v_vec] = fitmodel_mle_tit_f_tat_onepar_fix_share(trustee, reset_sub, ydata,condition, prior) % xdata1 is outcome, xdat2 is for resetting
% Call fminsearch with a random starting point.
% start_point = rand(1);
start_point = [10]; % i.e., HERE p(1) = alpha % p(2) = beta (inverse temperature)
model = @expfun;
