function [estimates, model, fval, exitflag, v_vec, delta] = fitmodel_mle_rl_BIN_bound_noreset(trustee, ydata, condition, prior) % trustee is outcome, xdat2 is for resetting
% Call fminsearch with a random starting point.
% start_point = rand(1);
start_point = [0.5, 0.5, 0.1]; % i.e., HERE p(1) = alpha % p(2) = beta (inverse temperature) 
model = @expfun;
        ...
        for i = 1:length(trustee)
            if i == 1
              ...
              switch  condition(i,1) %3nonsocial 4 social
                    case 4
                        v_vec(i,1) = prior; % keep rest fixed at 0.5 for BIN version
                    otherwise
                        v_vec(i,1) = 2.5; % keep rest fixed at 0.5 for BIN version
              end
            else
                if ydata(i-1,1) > 0 %if person shares just do your rl
                    delta(i,1) = trustee(i-1,1)*ydata(i-1,1)-v_vec(i-1,1);
