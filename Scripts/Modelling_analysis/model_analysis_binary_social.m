...
%           elseif i_mod == 7
%             n = size(A.investor,1);
%             [estimates_m{ i_mod }(i_sub,:), model, fval, exitflag(i_sub,i_mod), A.v_out] =  fitmodel_mle_tit_f_tat_onepar_fix_share( A.trustee, A.reset_sub, A.investor, A.condition, A.prior); % xdata1 is outcome
%             [LLE(i_sub,i_mod), FittedCurve2] = model(estimates_m{ i_mod }(i_sub,:)); 
%             AIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*1; % Akaike Information Criterion  % k1 = 1; % number of free parameters
%             BIC(i_sub,i_mod) = LLE(i_sub,i_mod) + 1*log(n)/2; % Bayesian Information Criterion
% %             to_plot = fitmodel_mle_tit_f_tat_fixed_to_plot(A.trustee, A.reset_sub, A.investor, estimates_m{ i_mod }(i_sub,:));
